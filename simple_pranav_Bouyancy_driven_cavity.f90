...
implicit none
    ...
    double precision :: gamma_u,gamma_v,gamma_T,Ra,Pr
    double precision :: relax_p,relax_u,relax_v,relax_t
...
implicit none
    ...
    iteration = 0
        do while(iteration<3)
              ...
              call u_correction();  call v_correction();  call pressure_correction()
...
implicit none
    ...
    do j= ny1_u+1,ny2_u-1
         do i= nx1_u+1,nx2_u-1
            ...
            De=gamma_u*dy(j)/dx(i+1)
            Dw=gamma_u*dy(j)/dx(i)
            Dn=gamma_u*dx_e(i)/dy_n(j)
            Ds=gamma_u*dx_e(i)/dy_n(j-1)
...
implicit none
	...
	do j= ny1_v+1,ny2_v-1
         do i= nx1_v+1,nx2_v-1
            ...
            De=gamma_v*dy_n(j)/dx_e(i)
            Dw=gamma_v*dy_n(j)/dx_e(i-1)
            Dn=gamma_v*dx(i)/dy(j+1)
            Ds=gamma_v*dx(i)/dy(j)
            ...
            Sc(i,j)=-((p(i,j+1)-p(i,j))/dy_n(j))+gamma_v*Ra*T(i,j)
            Sp(i,j)=0.0
    ...
    do j= ny1_T+1,ny2_T-1
        do i= nx1_T+1,nx2_T-1
           ! ae(i,j) = rho_e(i,j)*d_e(i,j)*dy(j)
            ...
            Fe=rho_e(i,j)*u_e*dy(j)
            Fw=rho_e(i-1,j)*u_w*dy(j)
            Fn=rho_n(i,j)*v_n*dx(i)
            Fs=rho_n(i,j-1)*v_s*dx(i)

            De=gamma_T*dy(j)/dx_e(i)
            Dw=gamma_T*dy(j)/dx_e(i-1)
            Dn=gamma_T*dx(i)/dy_n(j)
            Ds=gamma_T*dx(i)/dy_n(j-1)
            ...
            apo(i,j)=rho_old(i,j)/dt
            ap(i,j)=ae(i,j)+aw(i,j)+an(i,j)+as(i,j)+apo(i,j)*dx(i)*dy(j)&
                    -Sp(i,j)*dx(i)*dy(j)
            
            b(i,j)=Sc(i,j)*dx(i)*dy(j)+apo(i,j)*dx(i)*dy(j)*T_old(i,j)
...
integer :: i,j
  ...
  open(1, file='convection.data',Action ='write')
  open(2, file='streamfunctionconv.data',Action ='write')
  open(3, file='T_conv.data',Action ='write')
  open(4, file='v_velocityconv.data',Action ='write')
